...
1. Create the box — main clone, worktrees, ops folder, secrets, a tunnel,
   one shot:
   ...
   backend/.env and the ops folder up, then SSHes you into the main
   clone with localhost:8000/:5173 already tunneled. That first terminal
   is your orchestrator lane. -L is optional here — add it at `create`
   if you want the tunnel up immediately, or open it later (see below).
